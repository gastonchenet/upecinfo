...
import type { RawResource, RawSemester, Semester } from "../../types/Notes";
...
function parseResource(resource: RawResource) {
	return {
...
function parseSemesters(rawSemesters: RawSemester[]) {
	...
	rawSemesters.forEach((rawSemester) => {
		const semester: Semester = {
			...
			rank: parseInt(rawSemester.rank),
		...
		semester.ues = Object.entries(rawSemester.ues).map(([id, ue]) => ({
			id,
			title: ue.titre.replace(/S\d+/g, "").trim(),
			average: ue.moyenne.value === "~" ? 0 : parseFloat(ue.moyenne.value),
			bonus: parseFloat(ue.bonus),
			malus: parseFloat(ue.malus),
			groupSize: ue.moyenne.total,
			rank: parseInt(ue.moyenne.rang),
			note: parseFloat(ue.moyenne.value),
			min_note: parseFloat(ue.moyenne.min),
			max_note: parseFloat(ue.moyenne.max),
			evaluations: [
				...Object.keys(ue.ressources).map((id) => {
					const resource = semester.resources.find((r) => r.id === id);
					if (!resource) return [];
					return resource.evaluations;
				}),
				...Object.keys(ue.saes).map((id) => {
					const sae = semester.saes.find((s) => s.id === id);
					if (!sae) return [];
					return sae.evaluations;
				}),
			].flat(),
		}));
